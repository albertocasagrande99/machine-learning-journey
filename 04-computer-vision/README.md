...
| 5 | `05-clip-zero-shot-classification.ipynb` | Vision-Language Models (CLIP) | Deep dive into Contrastive Language-Image Pretraining (CLIP) and open-vocabulary zero-shot classification. Demonstrates feature extraction, manual similarity checks (using dot products), zero-shot classification plots, and image retrieval (text-to-image search). |
| 6 | `06-yolov8-pose-estimation-tracking.ipynb` | Pose Estimation & Tracking (YOLOv8 & ByteTRACK) | Extends detection to COCO 17-keypoint human pose estimation and multi-object tracking. Covers keypoint tensor parsing, custom skeleton rendering with Matplotlib, and persistent track ID association across video frames using ByteTRACK. |
